...
#NEED TO REMOVE SNP IF TOO MANY SAMPLES HAS ./.
# A MEMORY EFFICIENT METHOD, READ AND PROCESS ONE LINE EACH TIME
def species_specific_alleles(id_sp,sps,samples,snps,snps_info,min_freq,hqal,first):
  snp_al,snp_GT,sp_GT={},{},{}
  for sp in sps: #process each species
    ...
    #calculate GT frequency#tbh i am not interested in GT
    gts=[snps[sample]['GT'] for sample,spp in id_sp.items() if sp == spp['species'] and sample in samples]
    snp_GT[sp]={gt:round(100*gts.count(gt)/len(gts),2) for gt in set(gts)} #the dictionary with all GT frequencies for all SNPs
  tmp1={f"{species}_{al}": v.get(al,0) for species,v in snp_al.items() for al in [0,1,2,3,4]}
  ...
  ALs=f"{snps_info['REF']},{snps_info['ALT']}".split(',')
  #for al,typ in {0:'REF',1:'ALT'}.items(): #assume two alleles
  for al,typ in enumerate(ALs):
...
def slide_window_check(chrm,pos_dict,ws=1000,step=100):
  ...
  selected={k:{sp:list(v.values()).count(sp) for sp in sps} for k,v in bin_dict.items() if len(set(v.values())) >= min(max(stats),len(sps)-5)} #can lower the bar when there are more species, for 7 diploid salix no need to drop the bar, for all 12 sp: use 7
...
def specifi_SNPs(vcf,names,min_freq,output):
  ...
  for i in opener(vcf,'rt'):
    if i.startswith('#CHROM'):
      ...
      sps=list(set([sp['species'] for sample,sp in id_sp.items() if sample in samples and 'bad' not in sp['species']] )) #only consider species present in the sample. the sample-species list can be longer/contain more species than the one analysed
      ...
      if not i.startswith('#'): 
        ...
        first=species_specific_alleles(id_sp,sps,samples,snps,snps_info,int(min_freq),hqal,first) #output all species specific snps
...
def parse_ssSNP_csv(csv):
  ...
  for l in pos[1:]: #ignore the header
    l=l.split(',')
    chrm=l[1].split('|')[1]
...
def extract_ssSNP_regions(names,output):
  ...
  high_rsl_wds={f"{v['chrm']}_{i}":v for i,v in enumerate(high_rsl_wds)} #add index for gene array later
  df=pd.DataFrame.from_dict(high_rsl_wds,orient='index')
  # Keep first 3 columns fixed, sort the rest alphabetically
  first_cols = df.columns[:3].tolist()
  remaining_cols = sorted(df.columns[3:].tolist())
  df = df[first_cols + remaining_cols]
  print(df)
  df.to_csv(f'{output}_high_resolution_windows.csv', index=True) 
  print(f'Complete EXTRACTING REGION WITH SPECIES SPECIFIC SNPS to {output}_high_resolution_windows.csv')
